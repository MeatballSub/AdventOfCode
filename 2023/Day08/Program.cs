...
ProblemDesc parse(string file_name)
{
    ...
    List<Entry> nodes = lines[1].SplitLines()
                                .Select(_ => Regex.Match(_, @"(?<ele>\w+) = \((?<left>\w+), (?<right>\w+)\)"))
                                .Select(_ => (_.Groups["ele"].Value, _.Groups["left"].Value, _.Groups["right"].Value))
                                .ToList();
